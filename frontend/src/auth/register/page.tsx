...
export const RegisterPage = () => {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'El código ingresado es incorrecto o ha expirado.');
      }
      ...
      <div className="w-full lg:w-1/2 relative z-20 flex flex-col justify-center items-center p-6 sm:p-12 lg:p-24">
        <div className="w-full max-w-[440px]">
          ...
          {step === 'register' ? (
            ...
            <>
              <div className="mb-10 text-left">
                <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Verificación</h1>
                <p className="mt-2 text-slate-500 font-medium leading-relaxed">
                  Ingresa el código OTP de 6 dígitos que enviamos a tu correo <strong>{email}</strong> para activar tu cuenta.
                </p>
              </div>
              ...
              <form onSubmit={handleVerifyCode} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2 ml-1 text-center">Código de Verificación</label>
                  <input 
                    type="text" 
                    maxLength={6}
                    value={code}
                    ...
                    required
                    placeholder="123456"
                    className="w-full bg-white text-slate-900 text-center placeholder-slate-300 py-4 rounded-[1.25rem] text-[28px] font-black tracking-[8px] outline-none border-2 border-slate-200 focus:border-blue-500 hover:border-slate-300 transition-colors shadow-sm"
                  />
                </div>
